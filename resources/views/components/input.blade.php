...
<div class="mb-5 relative" @if(strtolower($type) === 'password') x-data="{ showPw: false }" @endif>
    <label for="{{ $id }}" class="text-black block text-[13px] font-medium mb-2 tracking-wide">{{ $label }}</label>
    ...
    @if(strtolower($type) === 'password')
        <div class="relative">
            <input type="password" 
                   ...
                   class="w-full pl-4 pr-12 py-3 bg-white/5 border border-white/5 rounded-xl text-black text-[15px] transition-all duration-250 outline-none placeholder:text-slate-500 placeholder:opacity-60 hover:bg-white/10 hover:border-white/10 focus:bg-white/10 focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/20"
